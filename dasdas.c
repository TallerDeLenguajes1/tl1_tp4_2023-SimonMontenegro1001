...
Tarea *BuscarTarea(Tarea **tareas, int cant, char *palabra);
...
void buscarEnTareasPendientes(Tarea **tareasPendientes, int cant) {
    ...
    char palabra[MAX];
    while (con) {
        puts("___buscador de tareas pendientes___\n");
        ...
        if (bus == 1) {
            ...
            gets(palabra);
            buscada = BuscarTarea(tareasPendientes, cant, palabra);
            if (buscada != NULL) {
                mostrarTarea(buscada);
            } else {
                puts("tarea no encontrada");
...
void buscarEnTareasRealizadas(Tarea **tareasrealizadas, int cant) {
    ...
    while (con) {
        ...
        if (bus == 1) {
            ...
            printf("ingresar palabra buscada: ");
            gets(palabra);
            buscada = BuscarTarea(tareasrealizadas, cant, palabra);
            if (buscada != NULL) {
                mostrarTarea(buscada);
            } else {
                puts("tarea no encontrada");
